...
///
/// Implements as much of the server-sent events format as the assistant uses:
...
Stream<ChatEvent> parseEvents(Stream<List<int>> bytes) {
  ...
  void dispatch() {
    ...
    try {
      ...
      out.addError(
        BadResponse(
          'The assistant sent an event I could not read.',
          body: '$payload ($e)',
        ),
      );
      return;
  ...
  void handleLine(String line) {
    ...
    switch (field) {
      case 'data':
        ...
        // newlines. The assistant sends one, but a conforming parser accepts both.
      ...
      case 'retry':
        ...
        // and the assistant never asks for a different retry interval. Parsed so
  ...
  void handleChunk(String chunk) {
    ...
    if (text.length > _maxFrameBytes) {
      out.addError(const BadResponse('The assistant sent an event that never ended.'));
      sub?.cancel();
      out.close();
      return;
    }
