...
/// AppButtonVariant : How much weight a button carries.
...
/// AppButton : The only button in the assistant.
///
...
class AppButton extends StatefulWidget {
  ...
  @override
  State<AppButton> createState() => _FButtonState();
}

class _FButtonState extends State<AppButton> {
  bool _hovered = false;
  bool _pressed = false;
  ...
  Widget build(BuildContext context) {
    ...
    final padding = widget.compact ? AppSpacing.md : AppSpacing.lg;
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Row(
              ...
              children: [
                if (widget.busy) ...[
                  AppSpinner(size: 14, color: foreground),
  ...
  (Color, Color, Color?) _colors(AppPalette c) {
    if (!_enabled) {
      ...
      return (
        c.surfaceRaised,
        c.textMuted,
        widget.variant == AppButtonVariant.ghost ? null : c.border,
      );
    }

    final lift = _pressed ? -0.06 : (_hovered ? 0.08 : 0.0);
    return switch (widget.variant) {
      AppButtonVariant.primary => (_shift(c.accent, lift), c.accentText, null),
      ...
      AppButtonVariant.danger => (_shift(c.danger, lift), Colors.white, null),
    };
  }
