/// The values every the assistant screen is built from.
///
...
/// `context.tokens` — so that light and dark differ only in the values, not
/// in the widgets that use them.
library;

import 'package:flutter/material.dart';

/// AppSpacing : The gaps. A small scale, because a large one only invites
...
/// AppMotion : How long things take.
///
...
/// AppBreakpoints : Where the layout changes shape.
...
/// AppPalette : Every colour a screen may use.
///
...
class AppPalette {
  ...
  /// accent : the assistant's own colour, used sparingly so that it still means
  ...
  /// dark : The default. the assistant is used at night and on a phone, and a dark
...
/// AppTypography : The text styles, by the job each does.
@immutable
class AppTypography {
  const AppTypography({
  ...
  /// mono : Anything the assistant returns that is code or an identifier.
  final TextStyle mono;

  /// of : The styles, coloured for a palette.
  factory AppTypography.of(AppPalette palette) {
    ...
    // behalf of whoever opens the app, and it fails outright on a network
...
/// AppTokens : The palette and type, carried on the theme.
///
...
class AppTokens extends ThemeExtension<AppTokens> {
  const AppTokens({required this.palette, required this.typography});
  ...
  static AppTokens of(BuildContext context) =>
      ...
      AppTokens(
        ...
        typography: AppTypography.of(AppPalette.dark),
      );
...
/// AppContext : Reaching the tokens without naming the theme each time.
extension AppContext on BuildContext {
  /// tokens : The tokens in scope.
  AppTokens get tokens => AppTokens.of(this);

  /// colors : The palette in scope.
  AppPalette get colors => AppTokens.of(this).palette;

  /// text : The type in scope.
  AppTypography get text => AppTokens.of(this).typography;
