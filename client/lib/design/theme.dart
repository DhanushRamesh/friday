/// Building Flutter's theme from FRIDAY's tokens.
library;
...
/// FTheme : The two themes, built from the two palettes.
///
/// Material's own ThemeData is filled in as well as the tokens, because
/// widgets FRIDAY does not own — a scrollbar, a text selection handle, a
/// tooltip — read from it and would otherwise arrive in Material's default
/// blue.
abstract final class FTheme {
  /// dark : The default.
  static ThemeData get dark => _build(FPalette.dark, Brightness.dark);

  /// light : For a bright room.
  static ThemeData get light => _build(FPalette.light, Brightness.light);

  static ThemeData _build(FPalette palette, Brightness brightness) {
    final typography = FTypography.of(palette);

    return ThemeData(
      useMaterial3: true,
      brightness: brightness,
      // So that a widget FRIDAY does not own — a tooltip, a menu — uses the
      ...
      extensions: [FTokens(palette: palette, typography: typography)],
      textTheme: TextTheme(
      ...
      // looks like part of FRIDAY rather than part of the browser.
      ...
      scrollbarTheme: ScrollbarThemeData(
        ...
        radius: const Radius.circular(FRadius.pill),
        crossAxisMargin: 2,
      ),
