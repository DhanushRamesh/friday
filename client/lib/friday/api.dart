...
/// FridayApi : A client for FRIDAY's HTTP interface.
///
...
class FridayApi {
  ...
  /// baseUrl : Where FRIDAY is, such as `https://friday-server.duckdns.org`.
  final Uri baseUrl;
  ...
  ///
  /// Returns as soon as FRIDAY accepts it, with the chat still pending —
  ...
  Stream<ChatEvent> streamChat(String chatId, {int resumeFrom = 0}) async* {
    ...
    try {
      ...
      throw Unreachable('I cannot reach FRIDAY at the moment.', e);
    }
  ...
  Future<Map<String, dynamic>> _send(
    ...
    try {
      ...
      throw Unreachable('FRIDAY took too long to answer.', e);
    } on Object catch (e) {
      throw Unreachable('I cannot reach FRIDAY at the moment.', e);
    }
    ...
    try {
      ...
      if (decoded is! Map<String, dynamic>) {
        throw BadResponse(
          'FRIDAY sent something unexpected.',
      ...
      throw BadResponse(
        'FRIDAY sent something I could not read.',
        body: _snippet(response.body),
      );
    }
  }
  ...
  /// read aloud.
  FridayException _failureFor(int status, String body) {
    ...
    return switch (status) {
      ...
      >= 400 && < 500 => Refused(
        message ?? 'FRIDAY would not accept that.',
        statusCode: status,
      ),
      _ => ServerFailure(
        message ?? 'Something went wrong on FRIDAY\'s end.',
        statusCode: status,
      ),
    };
  }
  ...
  String? _messageIn(String body) {
    ...
    try {
      ...
      // Not JSON, which happens when something in front of FRIDAY answers
      // instead of FRIDAY. There is nothing quotable in it.
    }
    return null;
  }
