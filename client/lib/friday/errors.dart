/// What can go wrong talking to FRIDAY.
library;

/// FridayException : The base of every failure this client reports.
///
...
sealed class FridayException implements Exception {
  const FridayException(this.message);
...
final class NotAuthenticated extends FridayException {
...
final class NotFound extends FridayException {
...
final class Refused extends FridayException {
...
final class Conflict extends FridayException {
  const Conflict([super.message = 'That has already finished.']);
}

/// ServerFailure : FRIDAY failed on its side. The cause is in its log, not
/// in this message, by design.
final class ServerFailure extends FridayException {
  const ServerFailure(super.message, {required this.statusCode});

  /// statusCode : What the server answered.
  final int statusCode;

  /// isRetryable : Whether trying again later could work. A 503 means FRIDAY
  /// is not accepting work at the moment, which passes.
  bool get isRetryable => statusCode == 503 || statusCode == 502;
}

/// Unreachable : FRIDAY could not be contacted at all — no network, no DNS,
...
final class Unreachable extends FridayException {
  const Unreachable([
    super.message = 'I cannot reach FRIDAY at the moment.',
    this.cause,
  ]);
...
final class BadResponse extends FridayException {
