...
| Memories are written **only when asked** | Owner's decision, with automatic extraction to be considered later behind a setting. A missing memory is noticed and can be added; a wrong one returns as fact, and nothing prompts anyone to doubt it. |
