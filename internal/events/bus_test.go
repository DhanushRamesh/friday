...
func TestSubscriberReceivesEventsForItsChat(t *testing.T) {
	b := newBus()
	ch, stop := b.Subscribe("chat_a")
	defer stop()

	b.Publish(events.Event{ChatID: "chat_a", Kind: events.KindUpdate, Seq: 1, Text: "working"})
...
// An event for one chat must not reach a listener for another.
func TestEventsAreScopedToTheirChat(t *testing.T) {
	b := newBus()
	chA, stopA := b.Subscribe("chat_a")
	defer stopA()
	chB, stopB := b.Subscribe("chat_b")
	defer stopB()

	b.Publish(events.Event{ChatID: "chat_a", Kind: events.KindUpdate, Seq: 1, Text: "for a"})
	...
	select {
	case ev := <-chB:
		t.Errorf("subscriber b received %+v, which belongs to another chat", ev)
	case <-time.After(50 * time.Millisecond):
	}
}

// Several clients may listen to one chat, as a phone and a desktop might.
func TestEveryListenerReceivesEveryEvent(t *testing.T) {
	...
	b.Publish(events.Event{ChatID: "chat_a", Kind: events.KindFinal, Text: "the answer"})
...
// Publishing must never block, or one stalled client holds up the chat that
// is producing the messages.
func TestPublishDoesNotBlockOnAStalledListener(t *testing.T) {
	b := newBus()
	_, stop := b.Subscribe("chat_a")
	defer stop()
	...
	go func() {
		...
		for i := 0; i < 5000; i++ {
			b.Publish(events.Event{ChatID: "chat_a", Kind: events.KindUpdate, Seq: i})
		}
	}()
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			for j := 0; j < 100; j++ {
				...
				b.Publish(events.Event{ChatID: "chat_a", Kind: events.KindUpdate, Seq: j})
				stop()
			}
		}()
	}
